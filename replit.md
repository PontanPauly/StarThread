...
2.  **Onboarding Flow**: Multi-step wizard for new users covering profile setup, family member addition (with temporary staging and duplicate detection), trusted contacts, and invite links.
...
-   **Identity Routes**: `server/routes/identity.js` — suggestions CRUD (`GET /api/identity/suggestions`, `POST .../accept`, `POST .../dismiss`), merge (`POST /api/identity/merge`), history (`GET /api/identity/merge-history`).
...
-   **Identity UI Components**: `src/components/identity/SuggestedMatches.jsx` (Home page), `src/components/identity/MergePersonDialog.jsx` (Settings page).

...
function MobileDpad({ keysRef }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener('contextmenu', clearAllKeys);

    return () => {
      document.removeEventListener('visibilitychange', handleVisibilityChange);
      window.removeEventListener('blur', clearAllKeys);
      window.removeEventListener('contextmenu', clearAllKeys);
      clearAllKeys();
    };
  }, [isTouchDevice, clearAllKeys]);
  ...
  const handlePointerUp = useCallback((key) => (e) => {
    e.preventDefault();
    e.stopPropagation();
    handleRelease(key);
  }, [handleRelease]);
  ...
  const btnBase = "flex items-center justify-center w-12 h-12 rounded-lg bg-slate-900/60 border border-amber-500/25 text-slate-300 active:bg-amber-500/30 active:text-amber-200 active:border-amber-400/50 select-none touch-none";

  return (
    <div className="absolute bottom-28 left-5 z-50 pointer-events-auto" style={{ touchAction: 'none' }}>
      <div className="grid grid-cols-3 grid-rows-3 gap-1" style={{ width: '156px', height: '156px' }}>
        <div />
        <button
          className={btnBase}
          onTouchStart={handlePointerDown('w')}
          onTouchEnd={handlePointerUp('w')}
          onTouchCancel={handlePointerUp('w')}
          onPointerDown={handlePointerDown('w')}
          onPointerUp={handlePointerUp('w')}
          onPointerLeave={handlePointerUp('w')}
          aria-label="Move forward"
        >
          <svg width="20" height="20" viewBox="0 0 20 20" fill="none"><path d="M10 4L16 14H4L10 4Z" fill="currentColor"/></svg>
        </button>
        <div />

        <button
          className={btnBase}
          onTouchStart={handlePointerDown('a')}
          onTouchEnd={handlePointerUp('a')}
          onTouchCancel={handlePointerUp('a')}
          onPointerDown={handlePointerDown('a')}
          onPointerUp={handlePointerUp('a')}
          onPointerLeave={handlePointerUp('a')}
          aria-label="Move left"
        >
          <svg width="20" height="20" viewBox="0 0 20 20" fill="none"><path d="M4 10L14 4V16L4 10Z" fill="currentColor"/></svg>
        </button>
        <div className="flex items-center justify-center w-12 h-12 rounded-lg bg-slate-900/30 border border-slate-700/20">
          <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><circle cx="8" cy="8" r="3" fill="currentColor" className="text-slate-600"/></svg>
        </div>
        <button
          className={btnBase}
          onTouchStart={handlePointerDown('d')}
          onTouchEnd={handlePointerUp('d')}
          onTouchCancel={handlePointerUp('d')}
          onPointerDown={handlePointerDown('d')}
          onPointerUp={handlePointerUp('d')}
          onPointerLeave={handlePointerUp('d')}
          aria-label="Move right"
        >
          <svg width="20" height="20" viewBox="0 0 20 20" fill="none"><path d="M16 10L6 4V16L16 10Z" fill="currentColor"/></svg>
        </button>
        ...
        <button
          ...
          onTouchEnd={handlePointerUp('s')}
          onTouchCancel={handlePointerUp('s')}
          onPointerDown={handlePointerDown('s')}
          onPointerUp={handlePointerUp('s')}
          onPointerLeave={handlePointerUp('s')}
          aria-label="Move backward"
        >
          <svg width="20" height="20" viewBox="0 0 20 20" fill="none"><path d="M10 16L4 6H16L10 16Z" fill="currentColor"/></svg>
        </button>
        <div />
      </div>
    </div>
